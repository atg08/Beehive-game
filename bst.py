...
class BinarySearchTree(Generic[K, I]):
    ...
    def traverse_left(self, current: TreeNode) -> TreeNode:
        if current.left == None:
            ...
            return self.traverse_left(current.left)
    ...
    def kth_smallest(self, k: int, current: TreeNode) -> TreeNode:
        ...
        while current:
            ...
            if current.left and k <= current.left.subtree_size:
                current = current.left

            elif current.left and k > current.left.subtree_size +1:
                k -= current.left.subtree_size + 1
